...
public class RocketJump extends Module {
    ...
    private void useRocket() {
        ...
        if (rocketSlot != ((dev.stardust.mixin.accessor.PlayerInventoryAccessor) mc.player.getInventory()).getSelectedSlot()) {
            if (rocketSlot < 9) InvUtils.swap(rocketSlot, true);
            else {
                InvUtils.move().from(rocketSlot).to(((dev.stardust.mixin.accessor.PlayerInventoryAccessor) mc.player.getInventory()).getSelectedSlot());
            }
        }
        ...
        else InvUtils.move().from(((dev.stardust.mixin.accessor.PlayerInventoryAccessor) mc.player.getInventory()).getSelectedSlot()).to(rocketSlot);
    }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        if (!jumped && wearingSomething) {
            ...
            if (preferChestplate.get()) {
                boolean found = false;
                if (chestplate != null) for (int n = 0; n < ((dev.stardust.mixin.accessor.PlayerInventoryAccessor) mc.player.getInventory()).getMain() /*private*/.size(); n++) {
                ...
                if (!found) for (int n = 0; n < ((dev.stardust.mixin.accessor.PlayerInventoryAccessor) mc.player.getInventory()).getMain() /*private*/.size(); n++) {
                    ItemStack stack = mc.player.getInventory().getStack(n);
