...
public class AutoSmith extends Module {
    ...
    private ArmorMaterial getArmorMaterial(ItemStack armor) {
        if (armor == null || armor.isEmpty()) return net.minecraft.item.equipment.ArmorMaterials.ARMADILLO_SCUTE;
    ...
    private boolean isValidEquipmentForTrimming(ItemStack stack) {
        ...
        boolean correctMaterial = switch (slot) {
            case 0 -> bootsType.get().materialEquals(armorMaterial);
            case 3 -> helmetType.get().materialEquals(armorMaterial);
            case 1 -> leggingsType.get().materialEquals(armorMaterial);
            case 2 -> chestplateType.get().materialEquals(armorMaterial);
            default -> false;
        };
    ...
    private boolean hasItem(Item needed) {
        ...
        for (int n = 0; n < ((dev.stardust.mixin.accessor.PlayerInventoryAccessor) mc.player.getInventory()).getMain().size() + 4; n++) {
            ...
            case Interact -> {
                ...
                switch (operatingMode.get()) {
                    case Trim -> {
                        ...
                        if (!output.isEmpty()) {
                            if (!(getItemSlotId(output) >= 0)) return;
                            ...
                            for (int n = 4; n < ((dev.stardust.mixin.accessor.PlayerInventoryAccessor) mc.player.getInventory()).getMain().size() + 4; n++) {
                                ItemStack stack = ss.getSlot(n).getStack();
                            ...
                            if (!(getItemSlotId(armorToTrim) >= 0)) {
                                foundEquip = false;
                                resettingTemplates = true;
    ...
    private @Nullable ClickSlotC2SPacket generateSmithingPacket(SmithingScreenHandler handler) {
        ...
        if (trimStack != null && materialStack != null && equipmentStack != null) {
            ...
            if (operatingMode.get().equals(SmithingMode.Trim) && (getItemSlotId(armorToTrim) < 0 || getItemSlotId(armorToTrim) > 3)) {
            ...
            if (operatingMode.get().equals(SmithingMode.Trim) && !trimStack.isOf(neededPattern)) {
                ...
                return makeClickPacket(handler.syncId, handler.getRevision(), (short) SmithingScreenHandler.TEMPLATE_ID, (byte) 0,
                    SlotActionType.QUICK_MOVE, changedSlots, ItemStack.EMPTY);
            }
            if (operatingMode.get().equals(SmithingMode.Trim) && !materialStack.isOf(neededMaterial)) {
                ...
                return makeClickPacket(handler.syncId, handler.getRevision(), (short) SmithingScreenHandler.MATERIAL_ID, (byte) 0,
                    SlotActionType.QUICK_MOVE, changedSlots, ItemStack.EMPTY);
            }
            ...
            equipmentStack = null;
            return makeClickPacket(handler.syncId, handler.getRevision(), (short) 3, (byte) 0,
                SlotActionType.QUICK_MOVE, changedSlots, ItemStack.EMPTY);
...
//                                .add(DataComponentTypes.TRIM, )
                    ...
                    return makeClickPacket(handler.syncId, handler.getRevision(), (short) n, (byte) 0,
                        SlotActionType.QUICK_MOVE, changedSlots, ItemStack.EMPTY);
                }
            }
    ...
    private ClickSlotC2SPacket makeClickPacket(int syncId, int revision, short slot, byte button,
                                               SlotActionType action, Int2ObjectMap<ItemStack> changed,
                                               ItemStack carried) {
        try {
            Class<?> cls = net.minecraft.network.packet.c2s.play.ClickSlotC2SPacket.class;
            for (java.lang.reflect.Constructor<?> c : cls.getDeclaredConstructors()) {
                Class<?>[] p = c.getParameterTypes();
                if (p.length != 7) continue;
                Object[] args = new Object[]{ syncId, revision, slot, button, action, (it.unimi.dsi.fastutil.ints.Int2ObjectMap) changed, carried };
                boolean ok = true;
                for (int i = 0; i < 7; i++) {
                    Object a = args[i];
                    if (a == null) continue;
                    if (!p[i].isInstance(a)) {
                        if ((p[i] == int.class && a instanceof Integer) || (p[i] == short.class && a instanceof Short) || (p[i] == byte.class && a instanceof Byte)) continue;
                        if (i == 6 && a instanceof ItemStack) continue;
                        ok = false; break;
                    }
                }
                if (!ok) continue;
                c.setAccessible(true);
                return (ClickSlotC2SPacket) c.newInstance(args);
            }
    ...
    private int predictEmptySlot(SmithingScreenHandler handler) {
        ...
        for (int n = ((dev.stardust.mixin.accessor.PlayerInventoryAccessor) mc.player.getInventory()).getMain().size() + 3; n >= 4; n--) {
