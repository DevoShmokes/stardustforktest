...
public class WaxAura extends Module {
    ...
    private void waxSign(SignBlockEntity sbe) {
        ...
        if (current.getItem() != Items.HONEYCOMB) {
            ...
            } else end = ((dev.stardust.mixin.accessor.PlayerInventoryAccessor) mc.player.getInventory()).getMain() /*private*/.size();
                ...
                synchronized (signsToWax) {
                    ...
                    if (signsToWax.isEmpty()) {
                        if (swapBack.get() && combSlot != -1) {
                            ...
                            else InvUtils.move().from(((dev.stardust.mixin.accessor.PlayerInventoryAccessor) mc.player.getInventory()).getSelectedSlot()).to(combSlot);
                            combSlot = -1;
                        }
                        return;
                    }
