...
public class Updraft extends Module {
    ...
    private void swapToWindCharge() {
        ...
        for (int n = 0; n < (hotBarSetting.get() ? 9 : ((dev.stardust.mixin.accessor.PlayerInventoryAccessor) mc.player.getInventory()).getMain() /*private*/.size()); n++) {
    ...
    private void swapFromWindCharge() {
        ...
        else InvUtils.move().from(((dev.stardust.mixin.accessor.PlayerInventoryAccessor) mc.player.getInventory()).getSelectedSlot()).to(returnSlot);
